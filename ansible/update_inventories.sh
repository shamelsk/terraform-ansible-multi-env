...
ANSIBLE_INVENTORY_DIR="/home/shamel/terra-ansible-mult-env/ansible/inventories"

# Navigate to the Terraform directory
cd "$TERRAFORM_OUTPUT_DIR" || { echo "Terraform directory not found"; exit 1; }

# Fetch IPs from Terraform outputs
DEV_IPS=$(terraform output -json dev_infra_instance_id | jq -r '.[]')
STG_IPS=$(terraform output -json stg_infra_instance_id | jq -r '.[]')
PRD_IPS=$(terraform output -json prd_infra_instance_id | jq -r '.[]')
...
update_inventory_file() {
    ...
    echo "ansible_ssh_private_key_file=/home/shamel/terra-ansible-mult-env/shamel-terra-key" >> "$inventory_file"
